...
import { deleteDonation } from "@/lib/firestore-service";
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1 bg-background rounded-xl p-1">
          {(["all", "today", "week", "month"] as FilterPeriod[]).map((f) => (
            ...
            >
              {f === "all" ? "All" : f === "week" ? "This Week" : f === "month" ? "This Month" : "Today"}
            </button>
          ))}
        </div>
      </div>
      ...
      {monthlyData.length > 0 && (
        <ChartCard title="Donation Trend" description="Monthly donation amounts" className="animate-fade-in">
      ...
      <div className="bg-white rounded-2xl border border-border overflow-hidden">
        {filtered.length === 0 ? (
          <EmptyState
            ...
            description={filter !== "all" ? "Try a different time filter." : "Donations will appear here once received."}
          />
        ) : (
